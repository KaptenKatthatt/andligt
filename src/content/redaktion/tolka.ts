...
const SEKTIONER: Record<string, 'öppning' | 'kärna' | 'historiskKontext'> = {
  'Öppning': 'öppning',
  'Kärna': 'kärna',
  'Historisk kontext': 'historiskKontext',
}
...
const delaFrontmatter = (fil: Innehallsfil): Tolkning<DeladFil> => {
  ...
  try {
    ...
    const beskrivning = orsak instanceof Error ? orsak.message : String(orsak)
    return { värde: null, fel: [`${fil.sökväg}: ogiltig yaml i frontmatter — ${beskrivning}`] }
  }
...
type Rumsfält = Partial<Record<'öppning' | 'kärna' | 'historiskKontext', string>>

const rumssektioner = (sökväg: string, kropp: string): Tolkning<Rumsfält> => {
  const fel: string[] = []
  const fält: Rumsfält = {}
  for (const [rubrik, text] of delaSektioner(kropp)) {
    const namn = SEKTIONER[rubrik]
    if (!namn) fel.push(`${sökväg}: okänd sektion "## ${rubrik}"`)
    else if (text.length > 0) fält[namn] = text
  }
  for (const krävd of ['Öppning', 'Kärna'] as const) {
    const namn = SEKTIONER[krävd]
    if (namn && fält[namn] === undefined) fel.push(`${sökväg}: saknar sektionen "## ${krävd}"`)
  }
...
/** Tolkar och validerar en enkel post (tema, fråga, källa, vandring …).
 * Brödtexten blir `beskrivning` när den inte är tom. */
export const tolkaPostfil = <T>(schema: z.ZodType<T>, fil: Innehallsfil): Tolkning<T> => {
  const delad = delaFrontmatter(fil)
  if (!delad.värde) return { värde: null, fel: delad.fel }
  const kropp = delad.värde.kropp.trim()
  const kandidat =
    kropp.length > 0 ? { ...delad.värde.frontmatter, beskrivning: kropp } : delad.värde.frontmatter
